				...
				<div class="col-12">
					...
					<div class="card card-primary">
						<div class="card-header">
							<h3 class="card-title" id="title_one">Emp</h3>
...
<script>
	...
	var ___datatable_list_one = $("#datatable_list_one").DataTable({
		...
		columns: [
			...
			,{ title: "Emp Name", data: "name", width: "20%" }
			,{ title: "Birth Day", data: "bday", width: "60px" }
			,{ title: "Sex ID", data: "sex_id", visible: false }
			,{ title: "Sex Description", data: "sex_description", width: "20%" }
			,{ title: "Edu Code", data: "edu_code" }
			,{ title: "Edu Description", data: "edu_description", width: "20%" }
			,{ title: "Alamat", data: "alamat", width: "20%" }
			,{ title: "Kota", data: "kota", width: "20%" }
			,{ title: "Provinsi", data: "provinsi", width: "20%" }
			,{ title: "Golongan Darah", data: "goldarah", width: "20%" }
			,{ title: "Reserved Words", data: "insertnew", width: "20%" }
		],
		buttons: {
			dom: {
				button: {
	...
	function create_one()
	{
		...
		$("#name, #alamat, #kota, #provinsi").removeClass("is-invalid");
		$("#nameError, #alamatError, #kotaError, #provinsiError").text("");
		$("#formtitle_one").html("Add Emp");	//two-1718
		$('#form_one')[0].reset();
		$("#form_one [name='sex_id']").val(null).trigger('change');	//three-2481
		$("#form_one [name='edu_code']").val(null).trigger('change');	//three-2481
		$("#form_one [name='goldarah']").val(null).trigger('change');	//three-2481
		$("#form_one [name='insertnew']").val(null).trigger('change');	//three-2481
		$("#modal_one").modal("show");	//two-1740
		__text_success1 = "Added!";
		__text_success2 = "Data Added Successfully!";
		__url = "{{ route('emp_savecreate_one') }}";	//store
	};
			...
			error: function(xhr) {
				if (xhr.status === 422) {
					...
					if (errors.kota) {
						...
						$('#kotaError').text(errors.kota[0]);
					}
					if (errors.provinsi) {
						$('#provinsi').addClass('is-invalid');
						$('#provinsiError').text(errors.provinsi[0]);
					}
				} else {
					Swal.fire({
						icon: 'error',
	...
	$(document).on('click', '.editIcon_one', function(e) {
		...
		$.ajax({
			...
			success: function(response) {
				$("#name, #alamat, #kota, #provinsi").removeClass("is-invalid");
				$("#nameError, #alamatError, #kotaError, #provinsiError").text("");
				$("#formtitle_one").html("Edit Emp");	//two-1965
				$("#modal_one").modal("show");
				__text_success1 = "Updated!";
				__text_success2 = "Data Updated Successfully!";
				__url = "{{ route('emp_saveedit_one') }}";	//update
				//action = "";	//two-1575
				$("#form_one [name='id']").val(response.id);	//three-426
				$("#form_one [name='name']").val(response.name);	//three-426
				$("#form_one [name='bday']").val(response.bday);	//three-426
				$("#form_one [name='sex_id']").val(response.sex_id).trigger('change');	//three-421
				$("#form_one [name='sex_description']").val(response.sex_description);	//three-426
				$("#form_one [name='edu_code']").val(response.edu_code).trigger('change');	//three-421
				$("#form_one [name='edu_description']").val(response.edu_description);	//three-426
				$("#form_one [name='alamat']").val(response.alamat);	//three-426
				$("#form_one [name='kota']").val(response.kota);	//three-426
				$("#form_one [name='provinsi']").val(response.provinsi);	//three-426
				$("#form_one [name='goldarah']").val(response.goldarah).trigger('change');	//three-421
				$("#form_one [name='insertnew']").val(response.insertnew).trigger('change');	//three-421
			}
		});
	});
	...
	$(document).on('click', '.deleteIcon_one', function(e) {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: '{{ route('employee-delete') }}',	//delete
					method: 'delete',
					data: {
					id: id,
					_token: csrf
